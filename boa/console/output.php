...
Document: http://boasoft.top/doc/#api/boa.console.output.html
...
class output extends base{
    ...
    public function table($body, $head = [], $border = false){
		$this->border = $border;
		$this->calc_rows($body);
		foreach($body as $row){
			$this->calc_cols($row);
	...
	private function calc_rows($data){
		foreach($data as $row => $line){
			foreach($line as $k => $v){
				$lines = substr_count($v, $this->wrap) + 1;
